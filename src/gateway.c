...
void request_gateway_mac(struct NetConfig *config) {
    ...
    if (DEBUG) {
        printf("Getting default gateway mac: gateway ip %s\n", inet_ntoa(gateway_ip));
    }
    ...
    uint8_t *packet = get_next_packet(handle, sizeof(struct ether_header) + sizeof(struct ether_arp));
    ...
    if (DEBUG) {
        printf("Getting default gateway mac: gateway mac: %s\n", mac_to_string(config->gateway_mac));
    }
